function deviceTypeListPage() {
    return {
        ...
        async init() {
            this.initTable();
            this.editModal = new bootstrap.Modal(document.getElementById('deviceTypeEditModal'));
        },
        ...
        async addItem() {
            ...
            try {
                await apiFetch('/api/device-types', { method: 'POST', body: JSON.stringify(this.newItem) });
        ...
        async saveEdit() {
            ...
            try {
                await apiFetch(`/api/device-types/${this.editItem.id}`, { method: 'PUT', body: JSON.stringify(this.editItem) });
                showToast(i18n.t('toast.saved'));
                this.editModal.hide();
                this.table.replaceData();
            } catch { }
        ...
        async deleteItem(id) {
            ...
            try {
                await apiFetch(`/api/device-types/${id}`, { method: 'DELETE' });
